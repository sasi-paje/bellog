...
| `invite-user` | Convida usuário: cria conta + registra em `master_system_user` + envia convite | JWT válido (autenticado) — gate fino é Fase 3 |
| `send-password-reset` | Reset de senha: gera link de recovery e envia (rate limit + anti-enumeração) | público (`verify_jwt=false`) |
| `test-smtp-connection` | Testa credenciais SMTP sem enviar email | JWT válido (autenticado) |
...
### Schema: master_system_user

A tabela **NÃO tem `id_auth_user`**. O elo com o Supabase Auth é o **`email`**. Colunas reais:
`id` (**bigint**), `email`, `full_name`, `id_user_role` (bigint, FK Cargo), `document_number`, `phone`,
`is_active`, `is_test`, `invitation_sent_at`, `invitation_accepted_at`, `last_login_at`, `created_at/by`, `updated_at/by`.
Login (`LoginPage`/`AdminAuthProvider`) sincroniza essa linha **por `(email, is_test)`** — localiza/atualiza `last_login_at` ou cria se não existir. **Nunca usar `id_auth_user`.**

### Roteamento de links (Vercel sem SPA fallback)

Rotas profundas dão **404** no Vercel (não há rewrite de SPA — ver `vercel.json`). Por isso os links de email e do mobile usam **query param na raiz**, não paths:
- Convite → `/?first_access=<email>` → `AdminApp` renderiza `FirstAccessPage`
- Reset → `/?reset_password=1` (+ `#access_token` no hash) → `AdminApp` renderiza `ForgotPasswordPage`
- Mobile (legado) → `/?page=...`

### Fluxo de convite + primeiro acesso (senha temporária)

1. **Usuários → Novo** → `UsersPage.handleSaveUser` → `inviteUser()` (`features/email/api/email.service.ts`) → `invite-user`.
2. `invite-user`: `auth.admin.createUser` com **senha temporária** (vira a senha real de login e é gravada em `user_metadata.temp_password`); registra em `master_system_user` por `(email, is_test)`; retorna `user_id = master_system_user.id`; envia email Bellog com **email + senha temporária + link `/?first_access=<email>`** via `send-email`/SMTP.
3. Convidado clica no link → `FirstAccessPage` em **modo standalone** (`?first_access`): informa a senha temporária + a nova senha → `signInWithPassword` (temp) → `updateUser` (define a definitiva, limpa `needs_password_change`/`temp_password`).
   - Alternativa: logar pelo `LoginPage` com a senha temporária → `needs_password_change` dispara o mesmo `FirstAccessPage`.

**Contratos:** `invite-user` retorna `user_id = master_system_user.id` (não o id do Auth); a linha nasce com o `is_test` enviado no payload, senão `saveUserPageAccessFromRole` falha com "Usuário não encontrado".
**Tradeoff:** a senha temporária trafega em texto no email e fica em `user_metadata` até a troca.

### Fluxo de reset de senha (link de recovery — sem senha temporária)

1. **Esqueci a senha** (`ForgotPasswordPage`, passo email) → `send-password-reset`.
2. `send-password-reset`: `auth.admin.generateLink({ type: 'recovery', redirectTo: \`${FRONTEND_URL}/?reset_password=1\` })` e envia o `action_link` via SES (Bellog). **Não** gera senha temporária nem altera a senha atual.
3. Clicar no link → Supabase valida e redireciona para `/?reset_password=1#access_token=...` → Supabase JS estabelece a sessão de recovery → `AdminApp` renderiza `ForgotPasswordPage` em **modo standalone** (passo "Nova Senha") → `updateUser` define a nova senha.

> **Dependência (Supabase → Authentication → URL Configuration):** o `redirectTo` precisa estar na allowlist de **Redirect URLs** (`https://bellog-trans.vercel.app/**`) e a **Site URL** apontar para o domínio de produção — senão o Supabase ignora o `redirectTo` e o `?reset_password` se perde.

### Permissões do convidado (RPC)

`saveUserPageAccessFromRole` chama `save_user_page_access_from_role(p_user_id **bigint**, ...)`.
O parâmetro era `text` e quebrava com `operator does not exist: bigint = text` — corrigido na migration `202606230002`. Passar `p_user_id` como **número**.

### Hardening aplicado

`invite-user` sanitiza email (rejeita CR/LF e formato inválido) e nome (strip CR/LF) contra injeção de cabeçalho SMTP/MIME. `email.service.ts` extrai a mensagem real do erro das edge functions (em vez do genérico "non-2xx status code").
