...
export const assignNotesService = {
   ...
   * Grupos são separados por papel da empresa:
   *   supplierGroups  → grupos de empresas com role SUPPLIER
  ...
  async getAdvancedFilterOptions(): Promise<{
    ...
    // Passo 1: buscar IDs dos role types de SUPPLIER e DESTINATION, mais endereços (paralelo)
    const [supplierRoleResult, destinationRoleResult, addressesResult] = await Promise.all([
      applyRefFilter(
        supabase
          .from('ref_person_company_role_type')
          .select('id')
    ...
    const supplierCompanyIds = [...new Set((supplierRelResult.data || []).map((r: any) => r.id_company).filter(Boolean))]
    ...
    const supplierGroups = toOptions(supplierGroupIds)
    ...
    return { supplierGroups, destinationGroups, cities, neighborhoods, neighborhoodsByCity: neighborhoodsByCityResult }
  },
  ...
  async getUnassignedNotes(params?: {
    ...
    if (params?.grupoCliente) {
      const roleIds = await getCompanyIdsByGroupAndRole(params.grupoCliente, 'SUPPLIER', isTest)
      if (params.razaoSocial && roleIds.length > 0) {
        const { data } = await supabase
          ...
          .in('id', roleIds)
        supplierIdsFilter = (data || []).map(s => String(s.id))
      } else {
        supplierIdsFilter = roleIds
      }
    } else if (params?.razaoSocial) {
      const { data } = await supabase
        .from('master_person_company')
        .select('id')
        .eq('is_active', true)
    ...
    const needsCustomerFilter = !!(params?.grupoDestino || params?.nomeDestino || params?.cidade || params?.bairro)
    if (needsCustomerFilter) {
      let byGroupName: string[] | null = null
      let byAddress: string[] | null = null

      if (params?.grupoDestino) {
        const roleIds = await getCompanyIdsByGroupAndRole(params.grupoDestino, 'DESTINATION', isTest)
      ...
      if (params?.cidade || params?.bairro) {
        let q = supabase
          .from('master_person_company_address')
          .select('id_company')
          .eq('is_active', true)
          .eq('is_test', isTest)
        if (params.cidade) q = q.eq('city', params.cidade)
        if (params.bairro) q = q.eq('district', params.bairro)
        const { data } = await q
