...
export const assignmentService = {
  ...
  async syncRouteInvoices(
    ...
    // After syncing invoices, also sync route stops
    await this.syncRouteStops(routeId, userId)
  },

  async syncRouteStops(routeId: string, userId?: string | number | null): Promise<{
    stops_created: number
    stops_existing: number
    invoices_linked: number
  }> {
    const routeIdNum = this.normalizeId(routeId)

    console.log('[syncRouteStops] routeId:', routeIdNum)

    const { data, error } = await supabase.rpc('sync_route_stops', {
      p_route_id: routeIdNum,
      p_user_id: userId ? this.normalizeId(userId) : null,
    })

    if (error) {
      console.error('[syncRouteStops] error:', error)
      throw new Error('Erro ao sincronizar paradas da rota: ' + error.message)
    }

    console.log('[syncRouteStops] result:', data)
    return data
  },
